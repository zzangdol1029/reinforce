"""
PDF 16페이지 Quiz Q2 — y = sin(4πx) 회귀
==========================================
...
RUNS: list[Run] = [
    Run(hidden=10,  lr=0.10, optimizer="sgd",      iters=10000),
    Run(hidden=32,  lr=0.10, optimizer="sgd",      iters=10000),
    Run(hidden=64,  lr=0.10, optimizer="sgd",      iters=10000),
    Run(hidden=32,  lr=0.20, optimizer="sgd",      iters=10000),
    Run(hidden=32,  lr=0.05, optimizer="sgd",      iters=10000),
    Run(hidden=32,  lr=0.10, optimizer="momentum", iters=10000),
    Run(hidden=32,  lr=0.10, optimizer="adagrad",  iters=10000),
    Run(hidden=32,  lr=0.01, optimizer="adam",     iters=10000),
]
# ──────────────────────────────────────────────────────────────────────────────
...
def run_compare(runs: list[Run], seed: int, n_samples: int, noise: float, save: str) -> None:
    ...
    y  = np.sin(4 * np.pi * x) + noise * np.random.rand(n_samples, 1)
    ...
    ax.plot(tt, np.sin(4 * np.pi * tt), "k--", lw=1.5, alpha=0.65,
            label=r"참값 $y=\sin(4\pi x)$", zorder=2)
...
def run_single(args: argparse.Namespace) -> None:
    ...
    y = np.sin(4 * np.pi * x) + args.noise * np.random.rand(args.n_samples, 1)
    ...
    plt.plot(tt, np.sin(4 * np.pi * tt), "k--", lw=1.2, alpha=0.75, label=r"$y=\sin(4\pi x)$")
    plt.xlabel("x"); plt.ylabel("y")
    plt.title("Q2 — sin(4πx) 회귀")
...
# ──────────────────── 진입점 ────────────────────
def main() -> None:
    ap = argparse.ArgumentParser(description="Q2: sin(4πx) 하이퍼파라미터 비교")
    ap.add_argument("--seed",      type=int,   default=SEED)
    ap.add_argument("--n-samples", type=int,   default=N_SAMPLES)
    ap.add_argument("--noise",     type=float, default=NOISE)
    ap.add_argument("--save",      type=str,   default="", help="저장할 파일명 (예: out.png)")
